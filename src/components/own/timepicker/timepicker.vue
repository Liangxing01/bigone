<template>
  <div>
    <input type="text" @click="openByDrop($event)" v-model="display">
    <div class="calendar-dropdown" :style="{'left':left+'px','top':top+'px'}" v-if="show">
        <calendar :range="range" @select="select"></calendar>
    </div>
  </div>
</template>
<script>
...
export default{
  ...
  data:function(){
    return {
      left:0,top:0,show:false,display:''
    }
  },
  methods:{
    select(a,b){
      this.show=false;
      if (this.range){
        this.display=a.join('-')+' ~ '+b.join('-');
        b=this.arr2str(b);
      }else this.display=a.join('-');
      a=this.arr2str(a);
    ...
    openByDrop(e){
        this.show=true;
        this.left=e.target.offsetLeft+19;
        this.top=e.target.offsetTop+70;
